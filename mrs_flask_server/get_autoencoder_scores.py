...
def get_plylsts_embeddings(_model_file_path, _submit_type, genre=False):
    if _submit_type == 'val':
        default_file_path = 'resources/static/data'
...
def save_scores(_autoencoder_embs, _score_type, _submit_type, genre=False):
    ...
    if genre:
        if _submit_type == 'val':
            np.save(f'resources/scores/val_scores_bias_{_score_type}_gnr', results)
        elif _submit_type == 'test':
            np.save(f'resources/scores/test_scores_bias_{_score_type}_gnr', results)
        else:
            np.save(f'resources/scores/local_val_scores_bias_{_score_type}_gnr', results)
    else:
        if _submit_type == 'val':
            np.save(f'resources/scores/val_scores_bias_{_score_type}', results)
        elif _submit_type == 'test':
            np.save(f'resources/scores/test_scores_bias_{_score_type}', results)
        else:
            np.save(f'resources/scores/local_val_scores_bias_{_score_type}', results)
