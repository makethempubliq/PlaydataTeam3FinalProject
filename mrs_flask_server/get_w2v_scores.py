...
def get_plylsts_embeddings(_train, _question, _submit_type):
    ...
    # toekenizer model
    tokenizer_name = 'resources/model/tokenizer_{}_{}_{}.model'.format(method, vocab_size, _submit_type)
    sp = spm.SentencePieceProcessor()
    sp.Load(tokenizer_name)
    tt = title_tokenizer()

    # w2v model
    w2v_model_name = 'resources/model/w2v_{}_{}_{}.model'.format(method, vocab_size, _submit_type)
...
def save_scores(_train, _question, _autoencoder_embs, _score_type, _submit_type):
    ...
    if _submit_type == 'val':
        np.save(f'resources/scores/val_scores_title_{_score_type}_24000', results)
    elif _submit_type == 'test':
        np.save(f'resources/scores/test_scores_title_{_score_type}_24000', results)
    elif _submit_type == 'local_val':
        np.save(f'resources/scores/local_val_scores_title_{_score_type}_24000', results)
    else:
        np.save(f'resources/scores/test_scores_title_{_score_type}_24000', results)
...
def get_w2v_scores(submit_type):
    if submit_type == 'val':
        ...
        question_file_path = 'resources/static/data/val.json'
        train_file_path = 'resources/static/data/train.json'
    elif submit_type == 'test':
        default_file_path = 'res'
        val_file_path = 'resources/static/data/val.json'
        question_file_path = 'resources/static/data/test.json'
        train_file_path = 'resources/static/data/train.json'
    ...
    genre_file_path = 'resources/static/data/genre_gn_all.json'

    tokenize_input_file_path = f'resources/model/tokenizer_input_{method}_{vocab_size}_{submit_type}.txt'
