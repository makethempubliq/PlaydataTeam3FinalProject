...
public class UserController {
    ...
    @GetMapping("/playlist")
    public String showplaylist(@RequestParam(value = "id", required = false) String playlistId, @RequestParam(value = "recommendedtracks", required = false) String recommendedtracks, @RequestParam(value = "tokenizedTheme", required = false) String themes, Authentication authentication, @AuthenticationPrincipal UserDetails userDetailsObj, Model model) throws Exception{
        ...
        if (playlistId != null){
            ...
            List<String> Tracks = new Gson().fromJson(recommendedtracks, new TypeToken<List<String>>() {}.getType());
            model.addAttribute("tracklist", Tracks);
            model.addAttribute("trackDataList", spotifyService.getTracksdata(userDetailsObj.getUsername(), Tracks));
            model.addAttribute("themes", new Gson().fromJson(themes, new TypeToken<List<String>>() {}.getType()));
        }
        model.addAttribute("accesstoken", accesstoken);
        return "playlist";
    }
    ...
    public String showplayerbar(@RequestParam String trackdata, Authentication authentication, @AuthenticationPrincipal UserDetails userDetailsObj, Model model) throws Exception{
    ...
    public String makeplaylist(Authentication authentication, @AuthenticationPrincipal UserDetails userDetailsObj, Model model) {
